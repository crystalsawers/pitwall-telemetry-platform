...
#=============================================
# Generate dashboard JSONs (no external files)
#=============================================

cat <<EOF > /tmp/f1-telemetry-data.json
{
  "annotations": {
    "list": [
      {
...
cat <<EOF > /tmp/f1-telemetry-system.json
{
  "annotations": {
    "list": [
      {
...
kubectl delete configmap grafana-dashboard-files -n default --ignore-not-found

kubectl create configmap grafana-dashboard-files \
  --from-file=f1-telemetry-data.json=/tmp/f1-telemetry-data.json \
  --from-file=f1-telemetry-system.json=/tmp/f1-telemetry-system.json \
  -n default

# ============================================
# ConfigMaps for Prometheus, Grafana, Loki and Promtail
# ============================================
cat <<EOF > $K8S_DIR/prometheus-config.yaml
apiVersion: v1
kind: ConfigMap
metadata:
...
# Loki configmap
cat <<EOF > $K8S_DIR/loki-config.yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: loki-config
...
# Promtail configmap
cat <<EOF > $K8S_DIR/promtail-config.yaml
apiVersion: v1
kind: ConfigMap
metadata:
...
# Grafana datasources and dashboard provider configmap
cat <<EOF > $K8S_DIR/grafana-datasources.yaml
apiVersion: v1
kind: ConfigMap
metadata:
...
cat <<EOF > $K8S_DIR/grafana-dashboard-provider.yaml
apiVersion: v1
kind: ConfigMap
metadata:
...
metadata:
  ...
  labels:
    app: "fastapi-app"
data:
  DATABASE_URL: "postgresql://fastapi-postgres:fastapipostgres79@postgres-db-service:5432/telemetry_db"
---
apiVersion: "apps/v1"
kind: "Deployment"
metadata:
